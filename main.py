...
def sex_tdr_corr(df):
    ...
    # T-test to see if there's a significant difference between test results of female and male
    gender_significance = stats.ttest_ind(female_values['tdr_paludisme_bool'], male_values['tdr_paludisme_bool'])
    print("Statistical results", gender_significance)
    print("Female average:")
    print(female_values['tdr_paludisme_bool'].mean())
    print("Male average:")
    print(male_values['tdr_paludisme_bool'].mean())
...
def age_tdr_corr(df):
    positive_values  = df[df['tdr_paludisme_bool'] == 1]
    negative_values = df[df['tdr_paludisme_bool'] == 0]
...
def hiv_tdr_corr(df):
    ...
    # more likely to provide positive TDR+ test results, but
...
if __name__ == '__main__':
    ...
    df['tdr_paludisme_bool'] = df['tdr_paludisme_string'].map(d)
    df = df[df['tdr_paludisme_bool'].notna()]
    weight_tdr_corr(df)
